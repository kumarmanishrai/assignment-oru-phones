  ...
  window.addEventListener("beforeunload", async () => {
    try {
      ...
      navigator.sendBeacon(
        `${process.env.NEXT_PUBLIC_API}/user/track-interaction`,
...
async function sendEvent(data: {
  ...
  if (data.eventType === "scroll") {
    ...
    let interactionTrackerData = JSON.parse(
      sessionStorage.getItem("interactionTrackerData") || "[]"
    );
    ...
    if (scrollIndex !== -1) {
      ...
      interactionTrackerData[scrollIndex] = {
        ...interactionTrackerData[scrollIndex],
        ...data,
      };
    } else {
    ...
    sessionStorage.setItem(
      "interactionTrackerData",
      JSON.stringify(interactionTrackerData)
    );
    console.log(data);
  } else {
    let interactionTrackerData = JSON.parse(
